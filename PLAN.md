...
- [x] **7D.2** Add `OPENAI_API_KEY_ENV_VAR`, `GOOGLE_API_KEY_ENV_VAR`, per-provider cost rate constants, `AI_DEFAULT_MODEL`, provider name constants, and model-prefix detection constants to `constants.py`
- [x] **7D.3** Define `AIProvider` protocol in `ai_review.py` with `call_review_api(prompt, model) -> tuple[str, int, int]`
- [x] **7D.4** Implement `_AnthropicProvider`, `_OpenAIProvider`, `_GoogleProvider` adapters — each ~30 lines, lazy import
- [x] **7D.5** Add `_detect_provider_name(model)` and `_get_provider(model, api_key)` factory in `ai_review.py`
...
- [x] **7D.7** Update `config.py`: add `enable_ai_review` key; emit `DeprecationWarning` for `enable_claude_review`; raise `ConfigurationError` if `anthropic_api_key` is set in YAML (use env var instead); add `model` field
- [x] **7D.8** Update `exceptions.py`, `help_text.py` — remove all Claude/Anthropic-specific language from user-facing text
- [x] **7D.9** Update tests: `test_ai_review.py` for provider-agnostic mocking; add provider-detection tests; add `test_config.py` multi-provider parsing tests
- [x] **7D.10** Update `phi-scan-action/action.yml` and `README.md`: rename `anthropic_api_key` input to `ai_api_key`; add `ai_model` input
